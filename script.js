...
async function getWeather() {
    try {
        ...
        forecastData.forEach((data) => {
            const day = new Date(data.dt * 1000).toLocaleDateString('en-US', { weekday: 'long', timeZone: 'Asia/Manila' });
        ...
        const extractedDateTime = new Date(date).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
        document.querySelector('.date-day').textContent = extractedDateTime;

        const currentWeatherIconCode = dailyForecast[today].icon;
        ...
        document.querySelector('.weather-desc').textContent = dailyForecast[today].description.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
